...
?>
<p><?php echo esc_html( $price_text_description ); ?></p>
<p><?php echo esc_html( $price_text ); ?> <input type="text" id="price" name="price" size="10" value="<?php esc_attr_e( $price ); ?>" style="width:auto;" /> <?php if ( !empty( $pmpro_currencies[$pmpro_currency]['position'] ) &&  $pmpro_currencies[$pmpro_currency]['position'] == 'right' ) { echo esc_html( $pmpro_currency_symbol ); } ?>
<span id="pmprovp-warning" class="pmpro_message pmpro_error" style="display:none;"><small><?php echo esc_html( $price_text_description ); ?></small></span></p>
<?php
}
